...
export function EntryCard({
  ...
  function expandEntryCard() {
    ...
    console.log(editHoursSpent, editNotes);
  }
  ...
  async function submitEditForm(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const editedEntry = Object.fromEntries(
        formData.entries()
      ) as unknown as Entry;
  ...
  return (
    <div className="calendar input-wrapper entrycard">
      <form
        className="calendar form-wrapper row-100"
        onSubmit={(event) => submitEditForm(event)}>
